...
/*   Updated: 2022/03/24 22:35:57 by baura            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_change_str(char *saved_strs)
{
	...
	str = (char *)malloc(sizeof(char) * (ft_strlen(saved_strs) - i + 1));
	if (!str)
		return (NULL);
	i++;
	while (saved_strs[i])
...
char	*ft_read_line(int fd, char *saved_strs)
{
	...
	if (buf == NULL)
		return (NULL);
	while (!ft_strchr_gnl(saved_strs, '\n') && ret != 0)
	{
		...
		buf[ret] = '\0';
		saved_strs = ft_strjoin_gnl(saved_strs, buf);
	}
	free (buf);
	return (saved_strs);
}
